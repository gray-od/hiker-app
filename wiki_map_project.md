...
### Раунд 22 — Видалення акаунту

- Кнопка "Видалити акаунт" в Настройках
- API route для каскадного видалення з усіх таблиць (gear_items, gear_lists, list_items, meal_plans, meal_days, meal_entries, user_food_items, ai_usage, profiles)
- Видалення auth.users через Supabase Admin API
- Підтвердження з введенням "ВИДАЛИТИ" або email
...
│   │   ├── privacy/page.tsx            # Privacy policy (i18n, GitHub Issues for deletion)
