...
### Раунд 23 (20.06.2026) — Pre-launch Hardening ✅

Комплексний аудит та виправлення перед публічним запуском:
- `src/app/not-found.tsx` — **новий**: брендована 404 сторінка (server component, getTranslations)
- `src/app/error.tsx` — **новий**: error boundary з кнопкою "Спробувати ще" (client, useTranslations)
- `src/app/global-error.tsx` — **новий**: root error boundary з `<html>`/`<body>` (hardcoded uk/en text)
- `src/app/layout.tsx` — додано `metadataBase`, `openGraph`, `twitter` мета-теги
- `src/app/opengraph-image.tsx` — **новий**: динамічна OG-картинка 1200×630 (gradient + logo + text)
- `src/app/robots.ts` — **новий**: дозвіл `/`, заборона внутрішніх сторінок
- `src/app/sitemap.ts` — **новий**: 3 публічні URL (/, /login, /privacy)
- `src/app/api/chat/route.ts` — `escapeLike()` для LIKE wildcard injection, видалено витік помилок БД (3 місця)
- `public/og-logo.png` — копія лого для OG-зображення
- i18n — +4 ключі "errors" × 3 локалі

**Аудит виявив:** 1 HIGH (LIKE injection), 3 MEDIUM (CSRF, rate limit delete, DB error leak), відсутність error pages та SEO. Виправлено все крім CSRF (SameSite cookies достатньо) та rate limit на delete (email confirm достатньо для Beta).

**Агенти:** debugger (error pages + i18n), code-reviewer (SEO files), security-auditor (chat route fixes)
**Build result:** ✅ TypeScript clean, JSON valid, 12 files changed

## File Structure (as of Round 23)
...
│   ├── og-logo.png                     # Logo for OG image generation
...
│   │   ├── error.tsx                   # Error boundary (client, useTranslations, reset button)
│   │   ├── global-error.tsx            # Root error boundary (hardcoded uk/en, own html/body)
│   │   ├── not-found.tsx               # Branded 404 page (server, getTranslations)
...
│   │   ├── layout.tsx                  # Root layout (i18n + ThemeProvider + PWA + Analytics + OG)
│   │   ├── manifest.ts                 # PWA manifest (name, theme_color, icons)
│   │   ├── opengraph-image.tsx         # Dynamic OG image (1200×630, logo + gradient)
│   │   ├── robots.ts                   # Search engine crawling rules
│   │   ├── sitemap.ts                  # Sitemap (3 public URLs)
